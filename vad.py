...
from model.silero import SileroVADModel
...
def get_speech_timestamps(audio: torch.Tensor,
    ...
    for current_start_sample in range(0, audio_length_samples, window_size_samples):
        ...
        speech_prob = model(chunk[None, ...], 16000).item()
...
if __name__ == '__main__':
    ...
    model.eval()
    model.load_state_dict(torch.load('./resource/silero_vad.pt'))
    audio = read_audio("./resource/test_audio.wav")
    print(get_speech_timestamps(audio, model, return_seconds=True))
    jit_model = torch.jit.load("./resource/silero_vad.jit")
    jit_model.eval()
    print(get_speech_timestamps(audio, jit_model, return_seconds=True))
